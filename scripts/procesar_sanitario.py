# Archivo: scripts/procesar_sanitario.py
...
# --- CONFIGURACIÓN ---
NOMBRE_PDF_ENTRADA = "Código Sanitario-Texto Consolidado.pdf"
DIRECTORIO_DATA = os.path.join(os.path.dirname(__file__), '..', 'data')
ARCHIVO_JSON_SALIDA = os.path.join(DIRECTORIO_DATA, 'sanitario_data.json')
...
def extraer_texto_pdf(ruta_pdf):
    ...
    try:
        ...
        texto_completo = ""
        # <-- CAMBIO: Empezamos desde la página 4 (índice 3) para este PDF
        for num_pagina in range(3, len(documento)):
...
def limpiar_texto(texto):
    ...
    textos_a_eliminar = [
        ...
        r"Texto consolidado de la Ley N° 836/1980",
...
def procesar_y_estructurar(texto_limpio):
    # <-- Este código es más simple, no tiene Libros
    contexto_actual = {'titulo': 'N/A', 'capitulo': 'N/A'}
    buffer_articulo = []
    ...
    for linea in lineas:
        ...
        es_marcador = match_titulo or match_capitulo or match_articulo
