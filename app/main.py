...
async def test_openai_connection():
    """Test de conexión con OpenAI con retry logic"""
...
Para consultas de otra naturaleza, diríjase a los servicios especializados correspondientes."""
        ...
        if contexto and isinstance(contexto, dict) and contexto.get("pageContent"):
            ...
            if es_valido and score_relevancia >= 0.2:
...
MAX_TOKENS_INPUT_CONTEXTO = 500
MAX_TOKENS_RESPUESTA = 300
MAX_TOKENS_SISTEMA = 180
MAX_TOKENS_ARTICULO_UNICO = 800
PRIORIDAD_COHERENCIA_JURIDICA = True
...
def validar_calidad_contexto(contexto: Optional[Dict], pregunta: str) -> tuple[bool, float]:
    ...
    try:
        ...
        if numero_pregunta:
            umbral_minimo = 0.1   # MUY permisivo para artículos específicos
            ...
            umbral_minimo = 0.2   # Permisivo para consultas generales
        
        contenido_minimo = len(texto_contexto.strip()) >= 30
...
def buscar_con_manejo_errores(pregunta: str, collection_name: str) -> Optional[Dict]:
    """Búsqueda robusta con múltiples métodos y cache"""
    ...
    if contexto_cached:
        logger.info("🚀 CACHE HIT - Contexto recuperado del cache")
...
def clasificar_consulta_inteligente(pregunta: str) -> str:
    """Clasificación inteligente básica"""
    pregunta_lower = pregunta.lower()
    scores = {}
...
async def clasificar_consulta_con_ia_robusta(pregunta: str) -> str:
    """Clasificación robusta con IA y cache"""
...
def truncar_contexto_inteligente(contexto: str, max_tokens: int = MAX_TOKENS_INPUT_CONTEXTO) -> str:
    """Truncado inteligente para contextos legales"""
    if not contexto:
        return ""
    ...
    # Análisis simple para mantener contenido importante
...
def generar_template_emergencia(pregunta: str, contexto: Optional[Dict] = None) -> str:
    """Genera respuesta usando templates de emergencia"""
    pregunta_lower = pregunta.lower()
    ...
    if contexto and contexto.get("pageContent"):
        ...
        contenido = contexto.get('pageContent', '')[:500]
...
**IMPORTANTE:** Esta respuesta fue generada por el sistema de emergencia de COLEPA.
...
def generar_respuesta_con_circuit_breaker(historial: List[MensajeChat], contexto: Optional[Dict] = None) -> str:
    """Generador con Circuit Breaker - GPT-4 → GPT-3.5 → Templates"""
    respuesta_cached = cache_manager.get_respuesta(historial, contexto)
    if respuesta_cached:
        return respuesta_cached
    
    if not OPENAI_AVAILABLE or not openai_client:
        resultado = generar_template_emergencia(historial[-1].content, contexto)
        cache_manager.set_respuesta(historial, contexto, resultado)
        return resultado
...
Responda en formato estructurado."""
        ...
        prompt_sin_contexto = f"""_clasificacion(self, pregunta: str, resultado: str):
    ...
    def set prompt_sin_contexto = f"""CONSULTA: {pregunta_actual[:150]}
